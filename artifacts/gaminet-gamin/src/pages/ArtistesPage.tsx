...
export default function ArtistesPage() {
  const { lang, t, translateField } = useLang();
  ...
  const [form, setForm] = useState({
    prenom: "", age: "", ville: "", courriel: "",
    telephone: "", raconte: "", consent1: false, consent2: false,
  });
  ...
  return (
    <main data-testid="artistes-page">
      ...
      <section className="relative min-h-[65vh] flex items-center bg-gradient-to-br from-amber-50 via-stone-50 to-green-50 overflow-hidden" data-testid="pa-hero">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20 relative z-10 w-full">
          <div className="max-w-2xl">
            <h1
              className="text-4xl sm:text-5xl md:text-6xl font-black text-stone-900 leading-tight tracking-tight mb-4 uppercase"
              style={{ fontFamily: "'Nunito', sans-serif" }}
              data-testid="artistes-title"
            >
              {t.artistes.titre}
            </h1>
            <p className="text-stone-600 text-lg leading-relaxed mb-6 max-w-xl" data-testid="artistes-sous-titre">
              {t.artistes.sous_titre}
            </p>
      ...
      {/* ── ARTISTES EN VEDETTE ── */}
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-20" data-testid="artistes-grid">
          ...
          {artistes.map((artiste, i) => (
            ...
            >
              <div className="flex items-start gap-4 mb-5">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center font-black text-3xl ${AVATAR_COLORS[i % AVATAR_COLORS.length]}`}>
                  {artiste.nom[0]}
                </div>
                <div>
                  <h2
                    ...
                    style={{ fontFamily: "'Nunito', sans-serif" }}
                    data-testid={`artiste-name-${artiste.id}`}
                  >
                    {artiste.nom}
                  </h2>
              ...
              <div className="flex gap-2 flex-wrap">
                {artiste.products.map((p) => (
                  <Link key={p.id} href={`${shopRoute}/${p.id}`}>
                    <div className="w-12 h-12 rounded-xl overflow-hidden border border-stone-100 hover:border-stone-300 transition-colors cursor-pointer" title={translateField(p.nom as any)}>
      ...
      {/* ── COMMENT ÇA MARCHE ── */}
      <section className="bg-stone-50 py-20" data-testid="pa-steps">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            {steps.map((step, i) => (
              <div key={i} className="relative" data-testid={`pa-step-${i + 1}`}>
                <div className="bg-white border border-stone-100 rounded-2xl p-8 text-center hover:shadow-md transition-all duration-300 h-full">
                  <div className="w-14 h-14 rounded-full bg-gg-gold/20 text-gg-gold flex items-center justify-center mx-auto mb-4">
                    {step.icon}
                  </div>
                  <div className="text-5xl font-black text-stone-100 absolute top-6 right-8" style={{ fontFamily: "'Nunito', sans-serif" }}>{i + 1}</div>
                  <h3 className="font-black text-stone-900 text-xl mb-3 relative z-10" style={{ fontFamily: "'Nunito', sans-serif" }}>{step.titre}</h3>
                  <p className="text-stone-500 text-sm leading-relaxed">{step.texte}</p>
                </div>
                {i < 2 && (
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-green-100 text-lg leading-relaxed mb-8">{pa.cause_texte}</p>
              <p className="text-green-200 text-sm leading-relaxed italic">{pa.cause_note}</p>
            </div>
            <div className="space-y-4">
              <p className="text-green-200 text-xs font-bold uppercase tracking-widest mb-4">{pa.cause_organismes}</p>
              {[pa.cause_org1, pa.cause_org2, pa.cause_org3].map((org, i) => (
                <div key={i} className="flex items-center gap-3 bg-white/10 rounded-xl px-5 py-4">
                  <div className="w-8 h-8 rounded-full bg-gg-gold/80 flex items-center justify-center text-stone-900 font-black text-sm flex-shrink-0">{i + 1}</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-white border border-stone-100 rounded-3xl p-8 sm:p-10 shadow-sm" data-testid="pa-form">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input type="number" name="age" required min={4} max={14} value={form.age} onChange={handleChange}
                  className="w-full border border-stone-200 rounded-xl px-4 py-3 text-stone-900 focus:outline-none focus:border-stone-900 transition-colors text-sm"
            ...
            <div>
              ...
              <input type="text" name="ville" required value={form.ville} onChange={handleChange}
                className="w-full border border-stone-200 rounded-xl px-4 py-3 text-stone-900 focus:outline-none focus:border-stone-900 transition-colors text-sm"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <input type="email" name="courriel" required value={form.courriel} onChange={handleChange}
                  className="w-full border border-stone-200 rounded-xl px-4 py-3 text-stone-900 focus:outline-none focus:border-stone-900 transition-colors text-sm"
            ...
            <div>
              ...
              <input ref={fileInputRef} type="file" accept="image/jpeg,image/png" onChange={handleFile} className="hidden" data-testid="input-file" />
            </div>
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest text-stone-500 mb-2">{pa.form_raconte}</label>
              <textarea name="raconte" value={form.raconte} onChange={handleChange} maxLength={200} rows={3}
            ...
            <div className="space-y-3 pt-2">
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" name="consent1" required checked={form.consent1} onChange={handleChange}
                  className="mt-0.5 w-4 h-4 accent-stone-900 flex-shrink-0" data-testid="input-consent1" />
                <span className="text-stone-600 text-sm leading-relaxed">{pa.form_consent1}</span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input type="checkbox" name="consent2" required checked={form.consent2} onChange={handleChange}
                  className="mt-0.5 w-4 h-4 accent-stone-900 flex-shrink-0" data-testid="input-consent2" />
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <span className="text-stone-400 text-sm ml-2">{s.age} {t.artistes.age}</span>
                    </div>
